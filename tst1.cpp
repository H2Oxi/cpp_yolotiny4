...
const string data_dir = "data_tst/conv_simple/int8/";
...
int main() {  
    ...
    int8_t in[3*6*6]; 
    int32_t bias[4];
    int8_t weights[4*3*3*3];
    int32_t Out[4*3*3];
    tensor_init(Out,4*3*3 );
    ...
    for (int i =0;i<3;i++)
    {
        for (int j=0;j<6;j++)
        {
            for (int k=0;k<6;k++)
            {
                std::cout<<static_cast<int>(in[(i*6+j)*6+k])<<" ";
            }
        std::cout<<endl;
        }
    std::cout<<endl;
    ...
    cout << "start!" <<endl;
    unsigned int in_shape_i[3]={3,6,6};

    conv_2d conv1_tst(&in_shape_i[0],4,3,2);
    conv1_tst.data_update(&weights[0],&bias[0]);

    conv1_tst.padding_forward(&in[0],&Out[0]);
    print_out_tensor(Out,4,3);
    
    save_int32_data(Out ,data_dir +"out_q.bin" ,4*3*3);
